...
void cadastrar(char nomes[][50], char emails[][30], float salarios[], int *qtd) {
    ...
    if (*qtd < 10) {
        ...
        // detalhe: usei scanf por escultar que nao era pra usar fgets( escutei laercio conversando com alguem sobre
        // usando o filtro "%49[^\n]" funciona perfeitamente sem o colateral do fgets de de guardar o caractere de
        // quebra de linha (\n) junto da string.
...
void buscar(char nomes[][50], char emails[][30], float salarios[], int qtd) {
    // aki tem alguns detalhes
    // 1. a variavel encontrado, funciona como um log, um retorno, basicamente ela vai ser responsavel por dizer
    // se o nome/cadastro nao foi encontrado
    // ela começa com 0 que significa  nao encontrado e so muda apos encontrar o cadastro
    int encontrado = 0;
...
void media(char nomes[][50], char emails[][30], float salarios[], int qtd) {
    if (qtd == 0) {
        ...
        // como temos essa varial qtd, que so e mudada quando e cadastrado 1 pessoa
...
int main(void) {
    ...
    do {
        ...
        // DETALHE: Dependendo do terminal que voce usar isso nao funciona, exemplo: se voce usar o terminal
        ...
        switch (op) {
            ...
            default:
                ...
                // basicamente o if, so esta deixando mais visivel oq esta acontecendo
                break;
        }
        printf("\nPressione Enter para continuar...");
        getchar();
        getchar();
        // explicação: como o loop so termina apos a pessoa digitar 6, ele meio que fica spamando, ele ja pula
        // para o menu, entao utilizei o getchar(afim da pessoa apertar enter), para que ele nao pule imediatamente
        // para o menu, pensei em usar sleep ( que so nao se aplica, pq eu estou usando linux pra programar, e provavelmente
        // laercio usa windows(resumindo nao sei onde vai ser testado), eu decidir usar o getchar ( que vai pegar 1 caracter)
        // basicamente um botao de continuar
    } while (op != 6);
